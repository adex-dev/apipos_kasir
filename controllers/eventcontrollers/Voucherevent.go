...
func Vouchereventt(c *fiber.Ctx) error {
	var promosi []isoide.VoucherModel
	rs, err := database.DBEVENT.Query(`SELECT p.id_promotion,p.type,p.nameP,p.discPersen,p.discValue,p.Minimal,p.Maximal,p.stdate,p.enddate,p.state,p.sttime,p.entime,p.outlet,p.addOn,p.member,p.status,user.nama_user from prm_discount_voucher as p inner join pos_master.users as user on user.nik = p.create_ons order by p.id_promotion DESC`)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"data": err.Error(), "respon": 501})
	}
	defer rs.Close()
	for rs.Next() {
		var prm isoide.VoucherModel
