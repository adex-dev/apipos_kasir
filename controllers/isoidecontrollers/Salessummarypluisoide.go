...
func Salessummarypluisoide(c *fiber.Ctx) error {
	var (
	...
	dt, err := SalesyPLUisoide(startdate, enddate, idstore)
	if err != nil {
...
func SalesyPLUisoide(startdate, enddate, idstore string) ([]isoide.Plusummary, error) {
...
func Dataprinterisoide(NoTrans string) ([]isoide.Dataprinter, error) {
	var dta []isoide.Dataprinter
	rows, err := database.DB.Query("SELECT price,qty,additional from printing_order where no_trans=? ", NoTrans)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
