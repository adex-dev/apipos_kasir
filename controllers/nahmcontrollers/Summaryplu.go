...
func Summaryplunahms(c *fiber.Ctx) error {
	var (
	...
	for _, vl := range dt {
		variables, err := dataprinternahm(vl.NoTrans)
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
				"error":  err,
				"data":   "dataprinter",
...
func dataprinternahm(NoTrans string) ([]isoide.Dataprinter, error) {
	var dta []isoide.Dataprinter
	rows, err := database.DBNAHM.Query("SELECT price,SUM(qty),additional from printing_order where no_trans=?", NoTrans)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
