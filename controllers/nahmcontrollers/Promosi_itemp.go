...
func Promosiitempp(c *fiber.Ctx) error {
	id := c.FormValue("id")
	var promositemp []isoide.Ptempmodel
	rs, err := database.DBNAHM.Query(`SELECT p.*,pt.nameP,pl.itemName  from  prm_discount_i_temp as p inner join prm_discount_i as pt on pt.id_promotion = p.idpromotion inner join plu_items as pl on pl.id_item=p.kd_product  where idpromotion=?`, id)
